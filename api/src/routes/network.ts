...
router.get("/", async (req, res) => {
  try {
    ...
    const activeNodes = nodes.filter(
      (node: any) => node.status === "ONLINE" && node.lastSeen && 
      new Date().getTime() - new Date(node.lastSeen).getTime() < 30000 // Online if seen in last 30 seconds
    ).length

    const completedJobs = allJobs.filter((job: any) => job.status === "COMPLETED").length

    // Calculate average accuracy from completed jobs
    const completedJobsWithResults = allJobs.filter(
      (job: any) => job.status === "COMPLETED" && job.result && typeof job.result === "object" && "accuracy" in job.result
    )
    
    const averageAccuracy = completedJobsWithResults.length > 0
      ? completedJobsWithResults.reduce((sum: number, job: any) => {
    ...
    const totalTokensStaked = allJobs.reduce((sum: number, job: any) => {
    ...
    const formattedNodes = nodes.map((node: any) => {
      ...
      const tokensEarned = contributions.reduce((sum: number, contrib: any) => {
    ...
    const recentJobs = allJobs.filter(
      (job: any) => new Date(job.createdAt) >= sixMonthsAgo
    )

    // Group jobs by month for timeline chart
    const jobTimelineData: Array<{ month: string; pending: number; inProgress: number; completed: number }> = []
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthJobs = recentJobs.filter((job: any) => {
      ...
      jobTimelineData.push({
        ...
        pending: monthJobs.filter((j: any) => j.status === "PENDING").length,
        inProgress: monthJobs.filter((j: any) => j.status === "RUNNING").length,
